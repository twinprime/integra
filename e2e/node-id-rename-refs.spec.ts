...
test.describe("renaming a node updates UseCase: and Sequence: refs in diagram content", () => {
  test("renaming a use-case node updates UseCase: path in referencing sequence diagram", async ({ page }) => {
    ...
    await page.getByRole("treeitem").filter({ hasText: /^Place Order$/ }).first().click()
  ...
  test("renaming a sequence diagram node updates Sequence: path in referencing diagram", async ({ page }) => {
    await page.addInitScript((value) => {
      localStorage.setItem("integra-system", value)
    }, makeLocalStorageValueWithSeqRef())
    await page.goto("/")
