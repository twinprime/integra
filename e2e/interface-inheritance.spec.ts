...
test.describe("interface inheritance — component-level inherit selector", () => {
  test("AuthService shows inherit-parent-select with only un-inherited parent interfaces", async ({ page }) => {
    await page.getByRole("treeitem").filter({ hasText: "AuthService" }).click()
  ...
  test("selecting a parent interface from inherit-parent-select creates a new tab", async ({ page }) => {
    await page.getByRole("treeitem").filter({ hasText: "AuthService" }).click()
    const select = page.getByTestId("inherit-parent-select")
