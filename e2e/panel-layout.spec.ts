...
test.describe('panel layout controls', () => {
    ...
    test('positions the horizontal splitter below the specification editor when opening a diagram', async ({
        page,
    }) => {
        ...
        expect(splitterToggleBox.y).toBeGreaterThanOrEqual(
            specificationBox.y + specificationBox.height - 1
        )
    })
    ...
    test('collapses and restores the editor pane while keeping visualization visible', async ({
        page,
    }) => {
    ...
    test('collapses and restores the visualization pane while keeping the editor visible', async ({
        page,
    }) => {
