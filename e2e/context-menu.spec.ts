...
test.describe("context menu", () => {
  test("opens on right-click of a component node", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Add Sub-component" }),
    ).toBeVisible()
    await expect(
      page.getByRole("button", { name: "Add Use Case Diagram" }),
    ).toBeVisible()
  })
  ...
  test("clicking Add Sub-component opens CreateNodeDialog", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Add Sub-component" }),
    ).toBeVisible()
  ...
  test("shows correct menu items per node type", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Add Sequence Diagram" }),
    ).not.toBeVisible()
