...
export function findOwnerUseCaseUuidById(ownerComp: ComponentNode, id: string): string | undefined {
...
export function resolveDiagramDeclarationUuid(
    ...
    if (keyword.startsWith('use')) {
        return (
            findOwnerUseCaseUuidById(ownerComp, id) ??
            findOwnerActorOrComponentUuidById(ownerComp, id)
        )
    }
...
function findFunctionReferenceTargetInTree(
    ...
    const fn = iface
        ? resolveEffectiveInterfaceFunctions(iface, current, treeRoot).find(
...
export function ensureScopedNodePath(
    ...
    if (parentSegments.length === 0) {
        ...
        if (!parentUuid) {
            const parentResult = ensureScopedNodePath(
                updatedRoot,
                parentSegments,
                'component',
                ownerUuid
    ...
    if (entityType === 'actor') {
        const newActor: ActorNode = {
            uuid: newUuid,
            id: terminalId,
            name: deriveNameFromId(terminalId),
            type: 'actor',
            description: '',
        }
