          ...
          interfaces: [
            {
              ...
              description: "",
              functions: [
                { uuid: "fn-alpha", id: "placeOrder", description: "", parameters: [] },
              ],
            },
          ],
        },
          ...
          interfaces: [
            {
              ...
              type: "rest",
              description: "",
              functions: [{ uuid: "fn-beta", id: "placeOrder", description: "", parameters: [] }],
            },
          ],
        },
      ],
...
describe("applyIdRename — extended scope regressions", () => {
  it("does not update ancestor-local references when renaming a child node with the same id", () => {
    const updated = applyIdRename(makeAncestorChildTree(), "uc-child", "placeOrder", "createOrder")
  ...
  it("keeps child-local references unchanged while updating explicit parent paths", () => {
    const updated = applyIdRename(makeAncestorChildTree(), "uc-root", "placeOrder", "createOrder")
    const childSequence = updated.subComponents[0].useCaseDiagrams[0].useCases[0].sequenceDiagrams[0].content
  ...
  it("does not update same-id function references in a sibling component", () => {
    const updated = applyIdRename(makeFunctionScopeTree(), "fn-alpha", "placeOrder", "createOrder")
    const alphaContent = updated.subComponents[0].useCaseDiagrams[0].useCases[0].sequenceDiagrams[0].content
    const betaContent = updated.subComponents[1].useCaseDiagrams[0].useCases[0].sequenceDiagrams[0].content
  ...
  it("updates only explicit sibling paths that resolve to the renamed target", () => {
    const updated = applyIdRename(makeSiblingPathTree(), "beta-sibling-uc", "placeOrder", "createOrder")
    const alphaContent = updated.subComponents[0].useCaseDiagrams[0].useCases[0].sequenceDiagrams[0].content
