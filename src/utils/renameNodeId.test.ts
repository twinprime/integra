...
describe('updateContentRefs', () => {
    ...
    it('replaces path segment in UseCase path reference', () => {
        expect(
            updateContentRefs(
                'UseCase:root/recorder/rec_stream:label',
                'recorder',
                'media_recorder'
            )
    ...
    it('replaces interface:function reference — interface rename', () => {
        expect(
            updateContentRefs('client->>api: OrdersAPI:place(a: string)', 'OrdersAPI', 'OrdersV2')
        ).toBe('client->>api: OrdersV2:place(a: string)')
    })

    it('replaces interface:function reference — function rename', () => {
        expect(
            updateContentRefs('client->>api: OrdersAPI:place(a: string)', 'place', 'placeOrder')
        ).toBe('client->>api: OrdersAPI:placeOrder(a: string)')
    })
...
describe('updateDescriptionRefs', () => {
    ...
    it('replaces ID as first path segment', () => {
        expect(updateDescriptionRefs('[Flow](login/main)', 'login', 'signIn')).toBe(
            '[Flow](signIn/main)'
        )
    })
