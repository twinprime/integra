import type { SystemNode, ComponentNode, ActorNode, UseCaseNode, InterfaceSpecification, Node } from '../store/types';

// Regex patterns
const ACTOR_PATTERN = /actor\s+"([^"]+)"\s+as\s+(\w+)/g;
const USE_CASE_PATTERN = /use case\s+"([^"]+)"\s+as\s+(\w+)/g;
// Mermaid sequence patterns
const PARTICIPANT_PATTERN = /participant\s+([^@\s]+)(?:@\{"type"\s*:\s*"([^"]+)"\})?/g;
const MESSAGE_PATTERN = /(\w+)\s*->>\s*(\w+)\s*:\s*(\w+)\(([^)]*)\)/g;

// Helper to find a component (or system) by ID
const findContainerInSystem = (system: SystemNode, id: string): ComponentNode | SystemNode | null => {
...
export function parseUseCaseDiagram(content: string, system: SystemNode, parentId: string): SystemNode {
    ...
    const mergeLists = <T extends { id: string, name: string }>(existing: T[], incoming: T[]): T[] => {
    ...
    // Construct new state
    const updateParent = (node: ComponentNode | SystemNode): ComponentNode | SystemNode => {
        return {
            ...node,
    ...
    const updateTree = (node: Node): Node => {
        ...
        if (node.type === 'component') {
            ...
            return {
                ...comp,
                subComponents: comp.subComponents.map(c => updateTree(c) as ComponentNode)
            };
        }
        return node;
    };
...
export function parseSequenceDiagram(content: string, system: SystemNode, parentId: string): SystemNode {
    const parent = findContainerInSystem(system, parentId);
    if (!parent) return system;

    const participants: { name: string; type: string }[] = [];
    const messages: { from: string; to: string; message: string; params: string }[] = [];

    let match;
    // Parse Participants
    while ((match = PARTICIPANT_PATTERN.exec(content)) !== null) {
        const [_, name, type] = match;
        participants.push({ name: name.trim(), type: type === 'actor' ? 'actor' : 'component' });
    }
    ...
    while ((match = MESSAGE_PATTERN.exec(content)) !== null) {
        messages.push({ from: match[1], to: match[2], message: match[3], params: match[4] });
    }

    // Infer participants from messages
    messages.forEach(msg => {
        if (!participants.find(p => p.name === msg.from)) participants.push({ name: msg.from, type: 'component' });
        if (!participants.find(p => p.name === msg.to)) participants.push({ name: msg.to, type: 'component' });
    });

    // Strategy: Calculate the NEW lists for the parent, then update tree.
    
    // 1. Identify Existing Components/Actors in Parent
    const existingComponents = parent.type === 'system' ? (parent as SystemNode).components : (parent as ComponentNode).subComponents;
    ...
    participants.forEach(p => {
        if (p.type === 'actor') {
            if (!updatedActors.find(a => a.id === p.name)) {
                updatedActors.push({ id: p.name, name: p.name, type: 'actor', description: '' });
            }
        } else {
             if (!updatedComponents.find(c => c.id === p.name)) {
                 updatedComponents.push({
                     id: p.name, name: p.name, type: 'component', description: '',
                     subComponents: [], actors: [], useCases: [], useCaseDiagrams: [], sequenceDiagrams: [], interfaces: []
                 });
             }
        }
    });
    ...
    messages.forEach(msg => {
        ...
        if (targetCompIndex >= 0) {
            ...
            let defaultInterfaceIndex = interfaces.findIndex(i => i.name === 'Default');
            ...
            if (!interactions.find(i => i.id === msg.message)) {
                interactions.push({
                    ...
                    parameters: msg.params ? msg.params.split(',').map(p => ({
                        name: p.trim(),
                        type: 'string',
                        required: true
                    })) : []
    ...
    const updateTree = (node: Node): Node => {
        if (node.id === parent.id) {
             if (node.type === 'system') {
                 return { ...node, components: updatedComponents, actors: updatedActors } as SystemNode;
             } else {
                 return { ...node, subComponents: updatedComponents, actors: updatedActors } as ComponentNode;
             }
        }
        ...
        if (node.type === 'system') {
             return {
                 ...node,
                 components: (node as SystemNode).components.map(c => updateTree(c) as ComponentNode)
             } as SystemNode;
        }
