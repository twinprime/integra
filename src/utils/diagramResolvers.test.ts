...
describe('ensureScopedNodePath', () => {
    ...
    it('returns null when target location is out of scope (cousin)', () => {
        ...
        const result = ensureScopedNodePath(
            root,
            ['sibling', 'cousin', 'target'],
            'component',
            'owner-uuid'
        )
...
describe('resolveFunctionReferenceTarget', () => {
    ...
    it('does not fall back outside the receiver subtree when that subtree already owns the interface', () => {
        const child = makeCompWithIface('child-uuid', 'child', 'API', 'child-iface-uuid')
        const svcA = makeCompWithFn(
            'a-uuid',
            'ServiceA',
