...
function makeRoot(ownerOverrides: Partial<ComponentNode> = {}, extraSubs: ComponentNode[] = []): ComponentNode {
  return {
    uuid: ROOT_UUID,
    id: "root",
    name: "Root",
...
describe("parseSequenceDiagram — participant declarations", () => {
  ...
  it("component participant with alias uses alias as name", () => {
    const root = makeRoot()
    const result = parseSequenceDiagram("component order_service as orders", root, OWNER_UUID, SEQ_UUID)
...
describe("parseSequenceDiagram — function call message", () => {
  ...
  it("kafka interface type assigns function to sender, not receiver", () => {
    ...
    const senderFn = sender!.interfaces.find((i) => i.id === "KafkaAPI")?.functions.find((f) => f.id === "publish")
    expect(senderFn).toBeDefined()
    // Receiver should NOT have gained the publish function
    const receiverFn = receiver!.interfaces.find((i) => i.id === "KafkaAPI")?.functions.find((f) => f.id === "publish")
...
describe("parseSequenceDiagram — cross-component path references", () => {
  it("multi-segment path reference adds target UUID to referencedNodeIds", () => {
    ...
    const result = parseSequenceDiagram("component root/auth as auth", root, OWNER_UUID, SEQ_UUID)
...
describe("parseSequenceDiagram — idempotency", () => {
  ...
  it("applying same function-ref message twice does not duplicate interface functions", () => {
    ...
    const iface = getOwner(after2).subComponents.find((c) => c.id === "orderSvc")!.interfaces.find((i) => i.id === "OrdersAPI")
