...
describe('sequence diagram parser — messages', () => {
    ...
    it('parses a message with a plain-text label', () => {
        ...
        expect(ast.messages[0]).toMatchObject({
            from: 'a',
            to: 'b',
            content: { kind: 'label', text: 'view running simulations' },
        })
    })
    ...
    it('parses a function ref with multiple parameters', () => {
        ...
        expect((ast.messages[0].content as { rawParams: string }).rawParams).toBe(
            'x: string, y: number'
        )
    })
...
adict ->> adict: stop event recording and remove recorded data`
        ...
        expect((ast.messages[2].content as { interfaceId: string }).interfaceId).toBe(
            'EventRecording'
        )
        expect((ast.messages[3].content as { text: string }).text).toBe(
            'stop event recording and remove recorded data'
        )
    })
})
...
describe('sequence diagram parser — notes', () => {
    it('parses note right of', () => {
        ...
        expect(ast.notes[0]).toMatchObject({
            position: { kind: 'side', side: 'right', participant: 'a' },
            text: 'some text',
        })
    })
...
describe('sequence diagram parser — multi-word participants', () => {
    it('parses a message with multi-word receiver', () => {
        ...
        expect(ast.messages[0]).toMatchObject({
            from: 'fts',
            to: 'Output Topics',
            content: { kind: 'label', text: 'Initial AIP data' },
        })
    })
