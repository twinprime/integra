...
describe('resolveUseCaseReferenceUuid', () => {
    ...
    const makeCompWithUcs = (uuid: string, id: string, ucIds: string[]): ComponentNode => ({
        ...
        useCaseDiagrams: [
            makeUcd(
                `${uuid}-ucd`,
                ucIds.map((ucId) => makeUc(`${uuid}-${ucId}-uuid`, ucId))
            ),
        ],
    })
    ...
    it('resolves use case in a sibling component by absolute path', () => {
        ...
        const result = resolveUseCaseReferenceUuid(
            ['orders', 'placeOrder'],
            root,
            owner,
            'owner-uuid'
    ...
    it('resolves a use case owned directly by an ancestor sibling component', () => {
        ...
        const parent = makeNamedComp('parent-uuid', 'parent', 'parent', [
            makeNamedComp('owner-uuid', 'owner', 'owner'),
            uncle,
        ])
        const root = makeNamedComp('root-uuid', 'root', 'root', [parent])
        const owner = parent.subComponents[0]
        const result = resolveUseCaseReferenceUuid(
            ['parent', 'uncle', 'placeOrder'],
            root,
            owner,
            'owner-uuid'
        )
...
describe('resolveSequenceReferenceUuid', () => {
    ...
    const makeCompWithSeqs = (
        uuid: string,
        id: string,
        seqIds: { id: string; name?: string }[]
    ): ComponentNode => ({
        uuid,
        id,
        name: id,
    ...
    it('resolves sequence diagram in a sibling component by absolute path', () => {
        ...
        const result = resolveSequenceReferenceUuid(
            ['auth', 'loginFlow'],
            root,
            owner,
            'owner-uuid'
