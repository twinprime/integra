...
export class SequenceDiagramParser extends CstParser {
    ...
    seqActivation = this.RULE('seqActivation', () => {
        this.OR([{ ALT: () => this.CONSUME(Activate) }, { ALT: () => this.CONSUME(Deactivate) }])
    ...
    seqDeclaration = this.RULE('seqDeclaration', () => {
        this.OR([{ ALT: () => this.CONSUME(Actor) }, { ALT: () => this.CONSUME(Component) }])
    ...
    nodePathSegment = this.RULE('nodePathSegment', () => {
        this.OR([{ ALT: () => this.CONSUME(Identifier) }, { ALT: () => this.CONSUME(NumberToken) }])
        this.MANY(() =>
            this.OR2([
    ...
    seqBlockSection = this.RULE('seqBlockSection', () => {
        ...
        this.OR([{ ALT: () => this.CONSUME(Else) }, { ALT: () => this.CONSUME(And) }])
