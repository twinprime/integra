...
function paramsMatch(a: ReadonlyArray<Parameter>, b: ReadonlyArray<Parameter>): boolean {
...
function applyFunctionToComponentByUuid(
  ...
  return upsertNodeInTree(root, uuid, (node) => {
    ...
    if (exactMatch === -1) {
      ...
      iface = { ...iface, functions: [...iface.functions, createInterfaceFunction(functionId, newParams)] }
    }
...
export function parseSequenceDiagram(
  ...
  if (astMessages.length > 0 && updatedOwnerComp) {
    let workingComponents: ReadonlyArray<ComponentNode> = [updatedOwnerComp, ...updatedOwnerComp.subComponents]
