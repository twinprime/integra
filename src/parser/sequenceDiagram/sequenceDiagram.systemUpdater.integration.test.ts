    ...
    it('throws when referencing a deep cousin (grandchild of sibling)', () => {
        ...
        expect(() =>
            parseSequenceDiagram(
                'component sibling/cousin/deepCousin as dc',
                root,
                ownerComp.uuid,
                'diag-uuid'
            )
...
describe('parseSequenceDiagram — auto-create missing path nodes', () => {
    it('auto-creates a missing sub-component when path parent exists', () => {
        ...
        const updated = parseSequenceDiagram(
            'component owner/newChild',
            root,
            ownerComp.uuid,
            'diag-uuid'
        )
    ...
    it('still throws for out-of-scope auto-create attempt (cousin path)', () => {
        ...
        expect(() =>
            parseSequenceDiagram(
                'component sibling/cousin/newDeepCousin',
                root,
                ownerComp.uuid,
                'diag-uuid'
            )
            ...
            ? [
                  {
                      ...
                      name: ifaceId,
                      type: 'rest' as const,
                      functions: [{ uuid: fnUuid, id: fnId, parameters: [] }],
                  },
              ]
            : []
    return {
...
describe('parseSequenceDiagram — function follows receiver', () => {
    it('adds function to new local receiver when receiver changes', () => {
        ...
        const serviceB = makeCompWithIface(
            'sb-uuid',
            'ServiceB',
            [],
            'REST',
        ...
        const cFn = updatedC.interfaces
            .find((i) => i.id === 'REST')
            ?.functions.find((f) => f.id === 'getUser')
    ...
    it('adds function to external (path) participant at the correct leaf component', () => {
        ...
        const fn = updatedServiceB.interfaces
            .find((i) => i.id === 'REST')
            ?.functions.find((f) => f.id === 'getUser')
    ...
    it('referencedFunctionUuids points to the function on the leaf external component', () => {
        ...
        const paymentFn = updatedPayment.interfaces
            .find((i) => i.id === 'REST')
            ?.functions.find((f) => f.id === 'getUser')
    ...
    it('tracks referencedFunctionUuids per receiver when sibling components share interface and function IDs', () => {
        ...
        const updatedDiagA = updatedOwnerA.useCaseDiagrams[0].useCases[0].sequenceDiagrams.find(
            (d) => d.uuid === 'diag-a-uuid'
        )!
        ...
        const updatedDiagB = updatedOwnerB.useCaseDiagrams[0].useCases[0].sequenceDiagrams.find(
            (d) => d.uuid === 'diag-b-uuid'
        )!
