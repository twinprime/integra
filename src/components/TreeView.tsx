...
export const TreeView = () => {
  ...
  const computeMenuItems = (node: Node): MenuItem[] => {
    ...
    if (node.type === "component") {
      ...
      items.push({ label: "Add Use Case Diagram", onClick: () => handleAddNode("use-case-diagram") })
    } else if (node.type === "use-case") {
      items.push({ label: "Add Sequence Diagram", onClick: () => handleAddNode("sequence-diagram") })
    }
    return items
  }
