...
const NodeLink = ({
    ...
    const isNodeLink =
        !!href && !href.includes('://') && !href.startsWith('#') && !href.startsWith('/')
...
export const MarkdownEditor = ({
    ...
    const NodeLinkWithContext = ({
        ...
        <NodeLink href={href} contextComponentUuid={contextComponentUuid}>
            {children}
        </NodeLink>
    )
    ...
    return (
        <div
            data-color-mode="dark"
            className={`[&_.w-md-editor-preview]:overflow-y-auto${className ? ` ${className}` : ''}`}
        >
            <MDEditor
                value={value}
