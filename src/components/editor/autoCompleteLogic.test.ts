...
describe('buildSuggestions — Sequence: suggestions', () => {
    ...
    it('suggests Sequence: for local sequence diagrams', () => {
        ...
        const owner = makeComp('owner-uuid', 'owner', {
            useCaseDiagrams: [makeUcd('ucd-uuid', [uc])],
        })
    ...
    it("suggests Sequence: with absolute path for remote component's sequence diagrams", () => {
        ...
        const seqSugg = suggs.find(
            (s) => s.insertText.startsWith('Sequence:') && s.insertText.endsWith('auth/loginFlow')
        )
        expect(seqSugg).toBeDefined()
    })
    ...
    it('suggests UseCase: and Sequence: targets from cousin components', () => {
        ...
        const cousin = makeComp('cousin-uuid', 'cousin', {
            useCaseDiagrams: [makeUcd('cousin-ucd-uuid', [cousinUc])],
        })
    ...
    it('suggests UseCase: and Sequence: even when receiver has no use cases (tree-wide search)', () => {
        ...
        const order = makeComp('order-uuid', 'OrderService', {
            useCaseDiagrams: [makeUcd('ucd-uuid', [uc])],
        })
        ...
        const suggs = buildSuggestions(ctx!, content, owner, root, 'sequence-diagram')
        const ucSugg = suggs.find(
            (s) => s.insertText.includes('UseCase:') && s.insertText.includes('placeOrder')
        )
        expect(ucSugg).toBeDefined()
        const seqSugg = suggs.find(
            (s) => s.insertText.includes('Sequence:') && s.insertText.includes('placeOrderFlow')
        )
        expect(seqSugg).toBeDefined()
    })
    ...
    it('for a root-owned sequence diagram, suggests both direct root-child and nested refs', () => {
        ...
        const nested = makeComp('nested-uuid', 'nested', {
            useCaseDiagrams: [makeUcd('nested-ucd-uuid', [nestedUc])],
        })
        ...
        const suggs = buildSuggestions(ctx!, content, root, root, 'sequence-diagram')
        expect(
            suggs.find((s) => s.insertText === 'UseCase:root/service/serviceUseCase')
        ).toBeDefined()
        expect(
...
describe('buildSuggestions — Sequence: inherited interface functions', () => {
    it("suggests functions from a parent interface when receiver's interface inherits", () => {
            ...
            functions: [
                {
                    uuid: 'fn-pay-uuid',
                    id: 'pay',
                    parameters: [{ uuid: 'p1', name: 'amount', type: 'decimal', required: false }],
                },
                {
                    uuid: 'fn-refund-uuid',
                    id: 'refund',
                    parameters: [{ uuid: 'p2', name: 'txId', type: 'string', required: false }],
                },
            ],
        }
        ...
        const suggs = buildSuggestions(ctx!, content, owner, root, 'sequence-diagram')
        expect(
            suggs.find((s) => s.insertText === 'PaymentService:pay(amount: decimal?)')
        ).toBeDefined()
        expect(
            suggs.find((s) => s.insertText === 'PaymentService:refund(txId: string?)')
        ).toBeDefined()
    })
    ...
    it('does not duplicate inherited functions when parent also appears as receiver', () => {
        ...
        const paySuggs = suggs.filter(
            (s) => s.insertText === 'PaymentService:pay(amount: decimal?)'
        )
...
describe('buildSuggestions — use case declaration', () => {
    ...
    it('inserts bare id (not quoted name-as format) for a local use case', () => {
        ...
        const owner = makeComp('owner-uuid', 'owner', {
            useCaseDiagrams: [makeUcdNode('ucd-uuid', [uc])],
        })
        const root = makeComp('root-uuid', 'root', { subComponents: [owner] })
    ...
    it('does not suggest use cases from other (non-owner) components', () => {
        ...
        const other = makeComp('other-uuid', 'other', {
            useCaseDiagrams: [makeUcdNode('ucd-uuid', [uc])],
        })
        const owner = makeComp('owner-uuid', 'owner')
