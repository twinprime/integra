...
import { collectDiagramsFromUcDiag, type DiagramRef } from './useCaseDiagramNode'
...
export const collectDiagramsFromComponent = (comp: ComponentNode): DiagramRef[] => {
    ...
    for (const ucd of comp.useCaseDiagrams) refs.push(...collectDiagramsFromUcDiag(ucd, comp.uuid))
    for (const sub of comp.subComponents) refs.push(...collectDiagramsFromComponent(sub))
    return refs
}
