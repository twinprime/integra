...
function makeUcd(overrides: Partial<UseCaseDiagramNode> = {}): UseCaseDiagramNode {
  return {
    uuid: "ucd1", id: "MainUCD", name: "Main UCD", type: "use-case-diagram",
...
function makeComp(uuid: string, id: string): ComponentNode {
  return { uuid, id, name: id, type: "component", subComponents: [], actors: [], useCaseDiagrams: [], interfaces: [] }
}
...
describe("deleteFromUcDiag", () => {
  ...
  it("also removes nested sequence diagrams within a use case", () => {
    const uc = { ...makeUseCase("uc1", "placeOrder"), sequenceDiagrams: [{ uuid: "sd1", id: "flow", name: "flow", type: "sequence-diagram" as const, content: "", referencedNodeIds: [], ownerComponentUuid: "c", referencedFunctionUuids: [] }] }
...
describe("upsertInUcDiag", () => {
  ...
  it("does not call updater when UUID does not match", () => {
    let called = false
    const ucd = makeUcd()
    upsertInUcDiag(ucd, "nonexistent", (n) => { called = true; return n })
...
describe("collectDiagramsFromUcDiag", () => {
  ...
  it("includes sequence diagrams from use cases", () => {
    const sd = { uuid: "sd1", id: "flow", name: "flow", type: "sequence-diagram" as const, content: "", referencedNodeIds: [], ownerComponentUuid: "c", referencedFunctionUuids: [] }
