...
import { applyIdRenameInUcDiag, deleteFromUcDiag } from './useCaseDiagramNode'
...
export const deleteFromComponent = (comp: ComponentNode, uuid: string): ComponentNode => ({
    ...comp,
...
export const upsertInComponent = (
    ...
    subComponents: comp.subComponents.map((c) =>
        c.uuid === uuid ? (updater(c) as ComponentNode) : upsertInComponent(c, uuid, updater)
    ),
    actors: comp.actors.map((a) => (a.uuid === uuid ? (updater(a) as ActorNode) : a)),
...
export const applyIdRenameInComponent = (
    ...
    actors: comp.actors.map((a) =>
        applyIdRenameInActor(a, targetUuid, oldId, newId, renameContext, comp.uuid)
    ),
