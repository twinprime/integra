 ...
 */
import type { ComponentNode, Node, ActorNode } from '../store/types'
import {
...
export const findOwnerComponentUuid = (root: ComponentNode, useCaseUuid: string): string | null =>
    findOwnerComponentUuidInComp(root, useCaseUuid)

// ─── Find container (component node by UUID) ──────────────────────────────────

export const findContainerInSystem = (root: ComponentNode, uuid: string): ComponentNode | null =>
    findContainerComponentByUuid(root, uuid)
...
export const reorderChildInParent = (
    ...
    upsertNodeInTree(root, parentUuid, (parent) => reorderInNode(parent, activeUuid, overUuid))
