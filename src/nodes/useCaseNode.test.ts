...
function makeSeqDiag(uuid: string, id: string): SequenceDiagramNode {
  return { uuid, id, name: id, type: "sequence-diagram", content: "", referencedNodeIds: [], ownerComponentUuid: "root", referencedFunctionUuids: [] }
}

function makeUseCase(overrides: Partial<UseCaseNode> = {}): UseCaseNode {
  return {
    uuid: "uc1", id: "placeOrder", name: "Place Order", type: "use-case",
...
describe("upsertInUseCase", () => {
  ...
  it("does not call updater when UUID does not match any diagram", () => {
    let called = false
    const uc = makeUseCase()
    upsertInUseCase(uc, "nonexistent", (n) => { called = true; return n })
...
describe("useCaseHandler", () => {
  ...
  it("addChild ignores non-sequence-diagram nodes", () => {
    const uc = makeUseCase({ sequenceDiagrams: [] })
    const nonSd = { uuid: "x", id: "x", name: "x", type: "use-case" as const, sequenceDiagrams: [] }
  ...
  it("addToComponent returns the component unchanged", () => {
    const comp = { uuid: "c", id: "c", name: "c", type: "component" as const, subComponents: [], actors: [], useCaseDiagrams: [], interfaces: [] }
