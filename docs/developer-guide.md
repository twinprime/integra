...
| Parent functions remain authoritative while child-local inherited functions are additive | Child-added functions may exist on inherited interfaces, but they cannot reuse a parent function ID; exact matches are removed explicitly through the conflict-resolution flow; adding a new function to a parent interface that conflicts with an existing child-local function triggers a `parent-add-conflict` prompt | `findInheritedParentFunction()`, `findInheritedParentFunctionById()`, `findChildFunctionsInInheritedInterfaces()`, the parser/update flow in `src/parser/sequenceDiagram/systemUpdater.ts`, and `applyFunctionUpdates()` |
...
7. If a new function is being added to a **parent** (local) interface but a child component's inherited version of that interface already has the same function ID with a different signature, a `parent-add-conflict` is emitted — the user confirms to add the parent function, remove the conflicting child-local definitions, and rebind all diagram references to the new parent function UUID via a full-system rebuild
